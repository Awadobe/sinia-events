-- One reliable update path for both migrated and newly created events.
-- Only the trusted server-side service role can execute it.
...
REVOKE ALL ON FUNCTION public.update_event_details(UUID, JSONB) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.update_event_details(UUID, JSONB) FROM anon;
REVOKE ALL ON FUNCTION public.update_event_details(UUID, JSONB) FROM authenticated;

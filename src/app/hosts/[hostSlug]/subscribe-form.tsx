...
export function SubscribeForm({ hostId, hostName, enabled }: { hostId: string; hostName: string; enabled: boolean }) {
    ...
    return (
        <div className="rounded-2xl bg-zinc-900 p-5 text-white sm:min-w-[330px]">
            <div className="flex items-center gap-2"><Bell className="h-4 w-4" /><p className="font-semibold">{enabled ? `Follow ${hostName}` : "Email updates coming soon"}</p></div>
            <p className="mt-1 text-xs leading-relaxed text-zinc-400">{enabled ? "Get an email when a new event is published. No account required." : `Soon you will be able to follow ${hostName} and receive new-event notifications.`}</p>
            {enabled && <>

    ...
    return (
        <main className="min-h-screen px-5 py-12 grid place-items-center" style={{ backgroundColor: isWedding ? design.background : "#fbf8f3" }}>
            <section className="w-full max-w-lg overflow-hidden rounded-[2rem] border border-stone-200 bg-white shadow-xl shadow-stone-200/40">
                {isWedding ? event.image_url ? <div className="relative aspect-[16/9]"><Image src={event.image_url} alt={`${event.title} invitation`} fill className="object-cover" unoptimized />{design.image_mode === "photo" && <div className="absolute inset-0 flex flex-col items-center justify-end bg-gradient-to-t from-black/80 via-transparent to-transparent p-6 text-center text-white"><p className="text-[10px] font-bold uppercase tracking-[0.25em]">{wedding.hosts || event.title} warmly invite</p><p className="mt-1 font-serif text-2xl font-semibold">{invitation.name || invitation.email.split("@")[0]}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-widest">Admits {invitation.party_size || 1}</p></div>}</div> : <div className="aspect-[16/8]"><WeddingInvitationPreview title={wedding.hosts || event.title} details={wedding} recipientName={invitation.name} partySize={invitation.party_size || 1} /></div> : <div className="bg-stone-900 px-8 py-7 text-center text-white"><p className="text-xs font-bold uppercase tracking-[0.28em]">Private invitation</p></div>}
                <div className="px-8 py-10 text-center">
                    <p className="text-sm text-stone-500">{inviter} invites {isWedding ? invitation.name || "you" : "you"} to</p>
